...
class RestrictedBoltzmanMachine(object):
    ...
    def __init__(self, num_visible, num_hidden, visible_unit_type='bin',
                 main_dir='/Users/az02234/Documents/Projet_AC/AnomalyDetectionAndLIME/model/', model_name='rbm_model',
                 gibbs_sampling_steps=1, learning_rate=0.01, momentum=0.9, l2=0.001, batch_size=10, num_epochs=10,
                 stddev=0.1, verbose=0, plot_training_loss=True, contamination=0.01):
    ...
    def _run_train_step(self, train_set):
        ...
        self.contamination_threshold = self.tf_session.run(self.contamination_threshold,
    ...
    def _build_model(self):
        ...
        self.contamination_threshold = tf.contrib.distributions.percentile(self.batch_free_energy, (1-self.contamination)*100)
    ...
    def _create_free_energy_for_bin(self):
        ...
        self.batch_free_energy = - (tf.matmul(self.input_data, tf.reshape(self.bv_, [-1, 1])) + tf.reshape(
    ...
    def predict(self, numpy_array):
        return self.getFreeEnergy(numpy_array) > self.contamination_threshold
